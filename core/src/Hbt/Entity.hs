...
newtype Shared = MkShared {unShared :: Last Bool}
...
getShared :: Shared -> Bool
getShared (MkShared value) = Maybe.fromMaybe False (getLast value)

newtype ToRead = MkToRead {unToRead :: Last Bool}
  deriving stock (Eq, Ord, Show, Generic)
  deriving newtype (Semigroup, Monoid, FromJSON, ToJSON)
...
getToRead :: ToRead -> Bool
getToRead (MkToRead value) = Maybe.fromMaybe False (getLast value)

newtype IsFeed = MkIsFeed {unIsFeed :: Any}
  deriving stock (Eq, Ord, Show, Generic)
  deriving newtype (Semigroup, Monoid, FromJSON, ToJSON)
