...
module Hbt.Collection.Internal
  ( Id (value)
  , Error (..)
  , Collection
  , CollectionRepr
...
import Hbt.Collection.Internal.Id (Id (..))
import Hbt.Collection.Internal.Repr (CollectionRepr (..), NodeRepr (..))
   ...
   in case (validFrom, validTo) of
        ...
        (True, True) ->
          let fromEdges = edgesAt from collection
           ...
           in collection {Hbt.Collection.Internal.edges = edges} -- lame. are we allowing duplicate records fields or not GHC?
