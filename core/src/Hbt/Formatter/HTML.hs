...
-- updated has no LAST_MODIFIED.
--
-- An anchor stating the same instant in ADD_DATE and LAST_MODIFIED does not
-- round-trip: 'Entity.normalize' drops the update at parse, so only ADD_DATE
-- comes back out. That is the normal form, not a formatting decision -- the
-- attribute names an update, and there was none. Fixture:
-- html/bookmarks_simple.
