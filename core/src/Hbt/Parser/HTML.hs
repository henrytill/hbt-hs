  ...
  case Text.toLower name of
    ...
    "tags" ->
      let tagList = Text.splitOn "," value
          ...
          toReadValue = Entity.getToRead entity.toRead || "toread" `elem` tagList
          toRead = Entity.mkToRead toReadValue
...
createEntity = do
  ...
  let startEntity = Entity.empty {shared = Entity.mkShared True}
